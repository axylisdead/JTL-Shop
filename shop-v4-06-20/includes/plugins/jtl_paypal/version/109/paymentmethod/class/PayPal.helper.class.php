...
class PayPalHelper
{
    ...
    public static function isLinked($paymentId)
    {
        $sql = 'SELECT count(tversandart.kVersandart) as cnt
            ...
            WHERE tversandartzahlungsart.kZahlungsart = ' . (int)$paymentId;
        $c = Shop::DB()->query($sql, 1);
        return is_object($c) && (int)$c->cnt > 0;
    }

    public static function copyCredentials($config, $prefix, $from, $to, $keys)
    {
        $copy = [];
        foreach ($keys as $key) {
            $fromKey = $prefix . $from . '_' . $key;
            $toKey = $prefix . $to . '_' . $key;
            if ((empty($config[$fromKey]) && !empty($config[$toKey])) || (!empty($config[$fromKey]) && empty($config[$toKey]))) {
                ...
                Shop::DB()->query("UPDATE tplugineinstellungen SET `cWert` = '{$v}' WHERE `cName` = '{$k}'", 3);
            }
        }
    }
