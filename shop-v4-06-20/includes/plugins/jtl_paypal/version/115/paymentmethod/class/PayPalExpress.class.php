...
class PayPalExpress extends PaymentMethod
{
    ...
    {
        return isset($_SESSION['reshash'])
            && !empty($_SESSION['reshash']['Token'])
            && !empty($_SESSION['reshash']['PayerID'])
            && parent::isValid($customer, $cart);
    }
    ...
    public function zahlungsprozess()
    {
        ...
        $this->addCache('initialOrderTotal', $basket->total[WarenkorbHelper::GROSS]);
    ...
    {
        if ($this->isValid($_SESSION['Kunde'], $_SESSION['Warenkorb'])) {
        ...
        switch ($response->Ack) {
            ...
            case 'Failure':
                ...
                if ((int)$error->ErrorCode === 10486) {
                    // The redirect will recreate the order - cancel current... SHOP-6560
                    Shop::DB()->update('tbestellung', 'kBestellung', $order->kBestellung, (object)[
                        'cStatus'    => BESTELLUNG_STATUS_STORNO,
                        'cAbgeholt'  => 'Y',
                        'cKommentar' => $this->oPlugin->oPluginSprachvariableAssoc_arr['jtl_paypal_payment_revoked_psd2overcharge'],
                    ]);
     ...
     * @return bool
     */
    public function checkOvercharge()
    {
        $basket       = PayPalHelper::getBasket();
        $initalTotal  = (float)$this->getCache('initialOrderTotal');
        $currentTotal = (float)$basket->total[WarenkorbHelper::GROSS];
        $maxOverpay   = (int)$this->oPlugin->oPluginEinstellungAssoc_arr[$this->pluginbez . '_psd2_overcharge_percent'];

        return $currentTotal > ($initalTotal * (1 + $maxOverpay / 100));
    }
}
