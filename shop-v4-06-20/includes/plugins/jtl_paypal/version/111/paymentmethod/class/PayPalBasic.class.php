...
class PayPalBasic extends PaymentMethod
{
    ...
    public function setExpressCheckout($helper, $return = false)
    {
        ...
        $languageISO     = PayPalHelper::getLanguageISO();
        $countryISO      = PayPalHelper::getCountryISO();
        $state           = PayPalHelper::getState($shippingAddress);
        ...
        $paymentAddress->StateOrProvince = $state;
    ...
    public function doExpressCheckoutPayment(BestellungHelper $helper, $args = [])
    {
        ...
        $token   = isset($args['token']) ? $args['token'] : $this->getCache('token');
