...
if (isset($_POST['stepPlugin']) && ($_POST['stepPlugin'] === 'neuesAttr' || $_POST['stepPlugin'] === 'alteAttr')) {
    if (isset($_POST['btn_delete'])) {
        ...
        $oNewAttribut->kVaterAttribut = isset($_POST['kVaterAttribut']) ? (int)$_POST['kVaterAttribut'] : 0;
        ...
        foreach ($_POST['eWertHerkunft'] as $key => $value) {
            ...
            $oOldAttribut->kAttribut = (int)$key;
            ...
            $oOldAttribut->kVaterAttribut = isset($_POST['kVaterAttribut'][$key])
                ? (int)$_POST['kVaterAttribut'][$key]
                : 0;
            ...
            if ($oPlausiReturn->bFehler === true) {
                ...
                $iRes = Shop::DB()->query(
                    ...
                    WHERE
                        kAttribut = {$oOldAttribut->kAttribut} {$cSqlStandard}",
                    3
                );
                if ($iRes === false) {
...
Shop::Smarty()->assign('attribute_arr', $attribute_arr)
    ...
    ->assign('URL_ADMINMENU', Shop::getURL() . '/' . PFAD_PLUGIN .
        $oPlugin->cVerzeichnis . '/' .
...
function plausiAttribut($oAttribut, $eWertHerkunft_arr, $oPlugin)
{
    ...
    if (!isset($oAttribut->kVaterAttribut) || !is_int($oAttribut->kVaterAttribut)) {
        ...
        $oRes = Shop::DB()->query("SELECT eWertHerkunft FROM xplugin_" . $oPlugin->cPluginID . "_attribut WHERE kAttribut = " . (int)$oAttribut->kVaterAttribut, 1);
