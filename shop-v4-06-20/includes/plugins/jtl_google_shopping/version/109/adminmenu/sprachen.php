...
if (isset($_POST['stepPlugin']) && $_POST['stepPlugin'] === $stepPlugin) {
    ...
    $oNewFeed->kSprache      = (int)$_POST['kSprache'];
    $oNewFeed->kKundengruppe = (int)$_POST['kKundengruppe'];
    $oNewFeed->kWaehrung     = (int)$_POST['kWaehrung'];
        ...
        if ($iRes > 0) {
            $oConf_arr = Shop::DB()->query(
                "SELECT cWertName
                    ...
                    WHERE kEinstellungenSektion = " . (int)CONF_EXPORTFORMATE, 2
            );
...
Shop::Smarty()->assign('oExportformate', $oExportformate)
    ...
    ->assign('URL_ADMINMENU', Shop::getURL() . '/' .
