        ...
        if (data.status === 'success') {
            ...
            if (typeof data.messages === "undefined") {
                if(typeof data.error === "undefined" || data.error === '') {
                    feedback = 'Es ist ein allgemeiner Fehler aufgetreten. Pr&uuml;fen Sie das Browser-Log und Systemlog.';
                } else {
                    feedback = 'Es ist folgender Fehler aufgetreten: ' + data.error;
                }
            } else {
                for (var index = 0; index < data.messages.length; ++index) {
                    feedback = feedback + data.messages[index] + '<br />';
