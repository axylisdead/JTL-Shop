...
// reset
    ...
    if (isset($_COOKIE[S360_LPA_ADDRESS_CONSENT_TOKEN_COOKIE])) {
        ...
        $getOrderReferenceDetailsParameter['address_consent_token'] = $token;
    }
    ...
    // Add article dependend costs to WK - this influences the possible shipping methods
    ...
    // We have to unset VersandKupon manually because JTL Warenkorb would only do this if another Kuponcode would be posted - see loescheSpezialPos
    // If we didn't do this, a free shipping Kupon would remain in the session even if it could not be applied to the given delivery address, making the shipping cost drop to 0!
    ...
    Shop::Smarty()->assign('oVersandart_arr', $allowed_delivery_methods)
        ->assign('lpa_shop_version', Shop::getVersion())
