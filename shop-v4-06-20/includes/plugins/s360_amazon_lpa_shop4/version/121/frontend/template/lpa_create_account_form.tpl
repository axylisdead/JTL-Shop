{if isset($lpa_billing_override) && $lpa_billing_override === 'Y'}
    {if $lpa_create_mode === "O" && $lpa_login_for_checkout}
        <div class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_create_description_optional_billing_override}</div>
    {elseif $lpa_create_mode === "N" && $lpa_login_for_checkout}
        <div class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_create_description_guest_billing_override}</div>
    {else}
        <div class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_create_description_billing_override}</div>
    {/if}
{else}
    {if $lpa_create_mode === "O" && $lpa_login_for_checkout}
        <div class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_create_description_optional}</div>
    {elseif $lpa_create_mode === "N" && $lpa_login_for_checkout}
        <div class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_create_description_guest}</div>
    {else}
...
<div class="alert alert-warning lpa-error-message" id="lpa-error-countrycode" style="display:none;">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_country_disallowed}</div>
<div class="alert alert-warning lpa-error-message" id="lpa-error-nonlatin" style="display:none;">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_nonlatin_error}</div>
        ...
        <form class="form" name="create_account" id="lpa-create-account-form" method="post" action="{$lpa_create_url_localized}">
            ...
            {if $Einstellungen.kunden.kundenregistrierung_abfragen_anrede!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_titel!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_firma!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_firmazusatz!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_adresszusatz!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_bundesland!="N" || $Einstellungen.kunden.kundenregistrierung_abfragen_ustid!="N"}
                <fieldset class="col-xs-12">
                    ...
                    {if !$lpa_template_mobile}
                        <legend>{lang key="address" section="account data"}</legend>{/if}
                    <div class="row">
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_anrede!="N"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.anrede>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.anrede>0}
                        ...
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_titel!="N" && $Einstellungen.kunden.kundenregistrierung_abfragen_titel=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.titel>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.titel>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                    </div>
                    ...
                    <div class="row">
                        ...
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_firma != 'N' && $Einstellungen.kunden.kundenregistrierung_abfragen_firma=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.firma>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.firma>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                        ...
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_firmazusatz != 'N' && $Einstellungen.kunden.kundenregistrierung_abfragen_firmazusatz=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.firmazusatz>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.firmazusatz>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                    </div>
                    ...
                    <div class="row">
                        ...
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_adresszusatz!="N" && $Einstellungen.kunden.kundenregistrierung_abfragen_adresszusatz=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.adresszusatz>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.adresszusatz>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                    </div>
                    ...
                    <div class="row">
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_bundesland!="N" && $Einstellungen.kunden.kundenregistrierung_abfragen_bundesland=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.bundesland>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.bundesland>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                    </div>
            ...
            {if $Einstellungen.kunden.kundenregistrierung_abfragen_fax=="Y" || $Einstellungen.kunden.kundenregistrierung_abfragen_tel=="Y" || $Einstellungen.kunden.kundenregistrierung_abfragen_mobil=="Y" || $Einstellungen.kunden.kundenregistrierung_abfragen_www=="Y" || $Einstellungen.kunden.kundenregistrierung_abfragen_geburtstag=="Y"}
                <fieldset class="col-xs-12">
                    {if !$lpa_template_mobile}
                        <legend>{lang key="contactInformation" section="account data"}</legend>{/if}
                    ...
                    <div class="row">
                        ...
                        {if $Einstellungen.kunden.kundenregistrierung_abfragen_www!="N" && $Einstellungen.kunden.kundenregistrierung_abfragen_www=="Y"}
                            <div class="col-xs-12 col-md-6 form-group required{if isset($fehlendeAngaben) && $fehlendeAngaben.www>0} has-error{/if}">
                                ...
                                {if isset($fehlendeAngaben) && $fehlendeAngaben.www>0}
                                    <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>{/if}
                            </div>
                        {/if}
                    </div>
            ...
            {if $lpa_create_mode === "O"}
                <div class="col-xs-12">
                    <div class="form-group">
                        <div class="checkbox">
                            <label class="control-label" for="lpa-create-account">
                                <input type="checkbox" name="lpa_create_account" value="1" id="lpa-create-account"{if $lpa_create_password_mode === "A"} onchange="lpa_toggle_passwords();"{/if}{if isset($lpa_create_account_selection) && $lpa_create_account_selection == 1} checked="checked"{/if}>
                                {lang key="createNewAccount" section="checkout"}
                            </label>
                        </div>
                    </div>
                </div>
            {/if}
            {if $lpa_create_password_mode === "A" && ($lpa_create_mode === "O" || $lpa_create_mode === "Y")}
                <fieldset class="col-xs-12" id="lpa-create-passwords" style="{if (isset($lpa_create_account_selection) && $lpa_create_account_selection == 1) || $lpa_create_mode === "Y"}max-height:none;visibility:visible;{else}max-height:0;visibility:hidden;{/if}">
                    <div class="row">
                        <div class="col-xs-12 col-md-6 form-group float-label-control{if isset($fehlendeAngaben.pass_zu_kurz) || isset($fehlendeAngaben.pass_ungleich)} has-error{/if}{if $lpa_create_mode === "Y"} required{/if}">
                            <label for="password" class="control-label">{lang key="password" section="account data"}</label>
                            <input type="password" name="pass" maxlength="20" id="password" class="form-control" placeholder="{lang key="password" section="account data"}"{if $lpa_create_mode === "Y"} required{/if}>
                            {if isset($fehlendeAngaben.pass_zu_kurz)}
                                <div class="alert alert-danger">{lang key="fillOut" section="global"}</div>
                            {/if}
                        </div>
                        <div class="col-xs-12 col-md-6 form-group float-label-control{if isset($fehlendeAngaben.pass_ungleich)} has-error{/if}{if $lpa_create_mode === "Y"} required{/if}">
                            <label for="password2" class="control-label">{lang key="passwordRepeat" section="account data"}</label>
                            <input type="password" name="pass2" maxlength="20" id="password2" class="form-control" placeholder="{lang key="passwordRepeat" section="account data"}"{if $lpa_create_mode === "Y"} required{/if}>
            ...
            <div class="col-xs-6 padded-lg-bottom padded-lg-top">
                <input type="submit" class="submit submit_once btn btn-primary" id="lpa-create-submit" style="visibility:hidden;" value="{lang key="sendCustomerData" section="account data"}"/>
            </div>
        </form>
    </div>
</div>