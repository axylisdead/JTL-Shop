...
require_once(__DIR__ . '/../../frontend/lib/lpa_includes.php');
...
$admAccount = new AdminAccount();

if (!$admAccount->logged()) {
    echo json_encode(array('status' => 'fail', 'messages' => array("Fehler - Admin-Login abgelaufen?")));
    exit;
}
...
if (empty($result)) {
    ...
    $kLinkgruppeHidden = (int) Shop::DB()->insert('tlinkgruppe', $obj);
    $userMessages[] = 'Linkgruppe "hidden" angelegt';
} else {
    $kLinkgruppeHidden = (int) $result[0]->kLinkgruppe;
}
...
if (!empty($result)) {
    foreach ($result as $link) {
        if ((int) $link->kLinkgruppe != $kLinkgruppeHidden) {
            /*
