<div id="amazonpayments" class="lpa-checkout-wrapper row">
    {if isset($cError) && $cError}
        <div class="col-xs-12">
            <p class="alert alert-warning">{$cError}</p>
        </div>
    {/if}
    {if isset($lpa_processing_error) && !$lpa_wassoftdecline}
        <div class="col-xs-12">
            <p class="alert alert-danger">{$lpa_processing_error.message}</p>
        </div>
    {/if}
    {if isset($lpa_not_logged_in) && $lpa_not_logged_in == 1}
        {* Error case - the checkout is rendered without a Kunde being in the session. *}
        <div class="col-xs-12">
            <p class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_not_logged_in}</p>
            <script type="text/javascript">
                jQuery(document).ready(function () {
                    lpa_logout();
                });
            </script>
            <div class="lpa-forced-pay-button"></div>
        </div>
    {elseif isset($lpa_is_valid_currency) && !$lpa_is_valid_currency}
        {* Error case - the checkout is rendered with an unsupported currency. Render buttons for the user to choose from. *}

        <div class="col-xs-12">
            <p class="alert alert-info">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_currency_hint_multicurrency}</p>
            <div class="btn-group" role="group">
                {foreach from=$lpa_available_supported_currencies item="lpa_currency"}
                    <a href="{$lpa_checkout_url_localized}?curr={$lpa_currency->cISO}" rel="nofollow" class="btn btn-default" title="{$lpa_currency->cName}">{$lpa_currency->cNameHTML}</a>
                {/foreach}
            </div>
        </div>
    {else}
        {if isset($lpa_billing_override) && $lpa_billing_override === 'Y' && $oPlugin->oPluginSprachvariableAssoc_arr.lpa_general_billing_override !== ''}
            <div class="col-xs-12 lpa-billing-address-override-hint">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">{lang key="billingAdress" section="account data"}</h3>
                    </div>
                    <div class="panel-body">
                        <p>
                            {$oPlugin->oPluginSprachvariableAssoc_arr.lpa_general_billing_override}
                        </p>
                    </div>
                </div>
            </div>
        {else}
            <div class="col-xs-12 lpa-billing-address">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">{lang key="billingAdress" section="account data"}</h3>
                    </div>
        ...
        {if !isset($confirmOrder)}
            ...
            *}
            <form method="post" action="{if isset($lpa_checkout_url_localized)}{$lpa_checkout_url_localized}{/if}">
                ...
                {/if}
                <div class="clear"></div>
                <div class="col-xs-12 col-sm-6 pull-right lpa-next" id="lpa-checkout-nextstep" style="display:none;">
                    <input type="hidden" name="lpa_step" value="lpaselected"/>
                    <input type="hidden" id="lpa-orid-input" name="orid" value=""/>
                    <input type="hidden" id="lpa-seller-id-input" name="seller_id" value="{if isset($lpa_seller_id)}{$lpa_seller_id}{/if}"/>
                    <input type="submit" value="{lang key="nextStepCheckout" section="checkout"}" class="btn btn-primary btn-lg submit submit_once"/>
                    <p>
                        <small>{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_check_order}</small>
                    </p>
                </div>
                <div class="col-xs-12 col-sm-6 pull-left lpa-cancel">
            ...
            <script type="text/javascript">
                ...
                window.walletInitFunc = function () {
                    new OffAmazonPayments.Widgets.Wallet({
                        ...
                        presentmentCurrency: '{/literal}{if isset($lpa_presentment_currency_iso)}{$lpa_presentment_currency_iso}{/if}{literal}',
                        design: {
        ...
        {else}
            ...
            <form method="post" action="" id="lpa-confirm-order-form">
                ...
                <div class="col-xs-12 col-md-6">
                    {if isset($lpa_processing_error) && $lpa_wassoftdecline}
                        <div class="box_error alert alert-danger" id="lpa-confirm-payment-message">{$lpa_processing_error.message}</div>
                    {else}
                        <div class="box_error alert alert-danger" id="lpa-confirm-payment-message" style="display:none;"></div>
                    {/if}
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            {if !$lpa_wassoftdecline}
                                <div id="readOnlyWalletWidgetDiv"></div>
                                <div id="editWalletWidgetDiv" style="display:none;"></div>
                            {else}
                                <input type="hidden" name="retryAuth" value="1" />
                                <div id="editWalletWidgetDiv"></div>
                            {/if}
                        </div>
                    </div>
                </div>
                ...
                <script type="text/javascript">
                    ...
                    var walletInitFunc = function () {
                        if($('#readOnlyWalletWidgetDiv').length) {
                            new OffAmazonPayments.Widgets.Wallet({
                                sellerId: '{/literal}{if isset($lpa_seller_id)}{$lpa_seller_id}{/if}{literal}',
                                amazonOrderReferenceId: '{/literal}{if isset($lpa_orid)}{$lpa_orid}{/if}{literal}',
                                displayMode: "Read",
                ...
                <div class="col-xs-12 col-md-6">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            {$oPlugin->oPluginSprachvariableAssoc_arr.lpa_amazon_pay_name}
                ...
                <div class="clear"></div>
                <div class="col-xs-12 col-sm-6 pull-right lpa-next">
                    <input type="hidden" name="finish" value="1"/>
                    <input type="hidden" id="lpa-orid-input" name="orid" value="{$lpa_orid}"/>
                    <input type="hidden" id="lpa-seller-id-input" name="seller_id" value="{if isset($lpa_seller_id)}{$lpa_seller_id}{/if}"/>
                    <input type="submit" value="{lang key="orderLiableToPay" section="checkout"}" id="complete-order-button" class="btn btn-primary btn-lg submit"{if $lpa_wassoftdecline} style="display:none;"{/if}/>
                </div>
                <div class="col-xs-12 col-sm-6 pull-left lpa-cancel">
                    <a href="warenkorb.php?{$SID}" onclick="lpa_logout();">{$oPlugin->oPluginSprachvariableAssoc_arr.lpa_cancel_checkout}</a>
                </div>
