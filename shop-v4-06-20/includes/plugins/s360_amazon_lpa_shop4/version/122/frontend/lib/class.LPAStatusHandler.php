 ...
 */
require_once(__DIR__ . '/lpa_includes.php');
require_once(PFAD_ROOT . PFAD_INCLUDES . "tools.Global.php");
require_once(PFAD_ROOT . PFAD_CLASSES . "class.JTL-Shop.Jtllog.php");
require_once(__DIR__ . '/lpa_defines.php');
require_once(__DIR__ . '/class.LPAController.php');
require_once(__DIR__ . '/class.LPADatabase.php');
require_once(__DIR__ . '/class.LPAAdapter.php');
require_once(__DIR__ . '/class.LPAAuthorization.php');
require_once(__DIR__ . '/class.LPACapture.php');
require_once(__DIR__ . '/class.LPAOrderReference.php');
require_once(__DIR__ . '/class.LPARefund.php');
require_once(__DIR__ . '/class.LPAHelper.php');
...
class LPAStatusHandler {
    ...
    public function handleOrderReference($newOrder) {
        ...
        if (!$oldOrder) {
            // Error state - unknown order reference id, but Amazon can not create payment objects that we do not know about.
            Jtllog::writeLog("LPA: Unbekannte OrderReference-ID empfangen: {$newOrder->cOrderReferenceId}", JTLLOG_LEVEL_DEBUG);
        ...
        // adjust kbestellung
        $newOrder->kBestellung = (int) $oldOrder->kBestellung;
        // adjust sandbox flag, it is per definition the same for the same payment object.
        $newOrder->bSandbox = (int) $oldOrder->bSandbox;
                    ...
                    if (!empty($auths) && is_array($auths)) {
                        ...
                        if ($realNewAuth->cAuthorizationStatus === S360_LPA_STATUS_OPEN ||
                            ...
                            if (!empty($billingaddress)) {
                                $rechnungsAdresse = LPAHelper::convertBillingAddressFromAmazonToJtl($billingaddress);
                            }
    ...
    public function handleAuthorization($newAuth) {
        ...
        if (!$oldAuth) {
            ...
            Jtllog::writeLog("LPA: Unbekannte Authorization-ID empfangen: {$newAuth->cAuthorizationId}", JTLLOG_LEVEL_DEBUG);
            return S360_LPA_ERROR_UNKNOWN_ID;
        }
        ...
        // adjust sandbox flag, it is per definition the same for the same payment object.
        $newAuth->bSandbox = (int) $oldAuth->bSandbox;
            ...
            case S360_LPA_STATUS_DECLINED:
                ...
                if ($oldAuth->cAuthorizationStatus !== S360_LPA_STATUS_DECLINED) {
                    switch ($newAuth->cAuthorizationStatusReason) {
                        ...
                        // should only be taken the first time that reason is encountered - after that, the authorization is forever DECLINED.
                        case S360_LPA_REASON_INVALID_PAYMENT_METHOD:
                        ...
                        case S360_LPA_REASON_AMAZON_REJECTED:
                            ...
                            $this->adapter->informBuyerAboutCancellation($newAuth->cOrderReferenceId);
                        ...
                        case S360_LPA_REASON_TRANSACTION_TIMED_OUT:
                            // the last authorization failed with a timeout, we do not retry an authorization but handle this like a hard decline
                            // we assume that all authorizations that get declined with a transaction timeout are asynchronous, so a retry is not feasible, as all async auths are done with the max timeout value.
                            // (Note hat synchronous authorizations that run into a timeout should not be known to us, anyway, because the order would not have been created, in the first place.)
                            Jtllog::writeLog("LPA: Authorization-ID {$newAuth->cAuthorizationId} - Status:  Zeitüberschreitung der Autorisierung. Order wird abgebrochen und storniert.", JTLLOG_LEVEL_NOTICE);
                            $this->adapter->informBuyerHardDecline($newAuth->cOrderReferenceId);
                            $this->db->setBestellungCanceled($newAuth->cOrderReferenceId);
                            $this->adapter->informBuyerAboutCancellation($newAuth->cOrderReferenceId);
                            $this->adapter->cancelOrder($newAuth->cOrderReferenceId, 'Authorization Timeout');
                            break;
                    }
                }
                $this->db->saveAuthorizationObject($newAuth);
                ...
                if ($oldAuth->cAuthorizationStatus !== S360_LPA_STATUS_CLOSED) {
                    switch ($newAuth->cAuthorizationStatusReason) {
                        case S360_LPA_REASON_EXPIRED_UNUSED:
                            $order = $this->db->getOrder($newAuth->cOrderReferenceId, true);
                            if ($order->cOrderStatus === S360_LPA_STATUS_OPEN && isset($order->bestellung, $order->bestellung->cStatus) && (int) $order->bestellung->cStatus != -1) {
                                ...
                                $this->adapter->authorize($order->cOrderReferenceId, (boolean) $newAuth->bCaptureNow);
                            } elseif(!isset($order->bestellung, $order->bestellung->cStatus) || (int) $order->bestellung->cStatus == -1) {
                                Jtllog::writeLog("LPA: Authorization-ID {$newAuth->cAuthorizationId} - Status:  Autorisierung abgelaufen! Versuche keine erneute Autorisierung, da Bestellung storniert oder unbekannt ist.", JTLLOG_LEVEL_NOTICE);
                            } else {
                        ...
                        case S360_LPA_REASON_MAX_CAPTURES_PROCESSED:
                            ...
                            break;
                        case S360_LPA_REASON_AMAZON_CLOSED:
                            Jtllog::writeLog("LPA: Authorization-ID {$newAuth->cAuthorizationId} - Status:  Autorisierung durch Amazon geschlossen.", JTLLOG_LEVEL_NOTICE);
                            break;
                        case S360_LPA_REASON_SELLER_CLOSED:
                            Jtllog::writeLog("LPA: Authorization-ID {$newAuth->cAuthorizationId} - Status:  Autorisierung durch Händler geschlossen.", JTLLOG_LEVEL_DEBUG);
                            break;
                        case S360_LPA_REASON_ORDER_REFERENCE_CANCELED:
                            Jtllog::writeLog("LPA: Authorization-ID {$newAuth->cAuthorizationId} - Status:  Autorisierung geschlossen, da Bestellung abgebrochen wurde.", JTLLOG_LEVEL_DEBUG);
                            break;
                    }
                }
                $this->db->saveAuthorizationObject($newAuth);
    ...
    public function handleCapture($newCap) {
        ...
        if (!$oldCap) {
            ...
            Jtllog::writeLog("LPA: Unbekannte Capture-ID empfangen: {$newCap->cCaptureId}", JTLLOG_LEVEL_DEBUG);
            return S360_LPA_ERROR_UNKNOWN_ID;
        }
        ...
        // adjust sandbox flag, it is per definition the same for the same payment object.
        $newCap->bSandbox = (int) $oldCap->bSandbox;
        ...
        switch ($newCap->cCaptureStatus) {
            ...
            case S360_LPA_STATUS_COMPLETED:
                ...
                $this->db->saveCaptureObject($newCap);
                $auth = $this->db->getAuthorization($newCap->cAuthorizationId);
                $this->db->setBestellungCaptured($auth->cOrderReferenceId, $newCap);


                /*
                 * We can close the order against amazon now if it has been paid completely.
                 */
                try {
                    if ($this->db->isOrderPaidCompletely($auth->cOrderReferenceId)) {
                        $this->adapter->setOrderCompleted($auth->cOrderReferenceId);
                    }
                } catch(Exception $ex) {
                    Jtllog::writeLog("LPA: Exception beim Versuch, Order automatisch zu schliessen: " . $ex->getMessage() . "(".$ex->getCode().")", JTLLOG_LEVEL_NOTICE);
                }
                break;
            case S360_LPA_STATUS_CLOSED:
                /*
    ...
    public function handleRefund($newRef) {
        ...
        if (!$oldRef) {
            ...
            Jtllog::writeLog("LPA: Unbekannte Refund-ID empfangen: {$newRef->cRefundId}", JTLLOG_LEVEL_DEBUG);
            return S360_LPA_ERROR_UNKNOWN_ID;
        }
        ...
        // adjust sandbox flag, it is per definition the same for the same payment object.
        $newRef->bSandbox = (int) $oldRef->bSandbox;
    ...
    private function checkUnchanged($old, $new, $type) {
        ...
        switch ($type) {
            ...
            case 'auth':
                ...
                $result = $result && ((float) $old->fCapturedAmount === (float) $new->fCapturedAmount);
            ...
            case 'cap':
                ...
                $result = $result && ((float) $old->fRefundedAmount === (float) $new->fRefundedAmount);
