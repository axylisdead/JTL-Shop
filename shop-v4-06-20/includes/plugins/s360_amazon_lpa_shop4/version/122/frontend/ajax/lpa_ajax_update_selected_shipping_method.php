...
// Shipping method specific cart positions should be reset
    ...
    // after this point the checksum of the basket should not change anymore until confirmed, therefore we remember it now and verify it in lpa_ajax_confirm_order.php
    // Note that we do not really care about the products in the basket but only about the total sum and the selected shipping method - these must not change.
    $_SESSION['lpa_basket_checksum'] = md5(serialize($_SESSION['Warenkorb']->gibGesamtsummeWaren(true) . $_SESSION['Versandart']->kVersandart));
