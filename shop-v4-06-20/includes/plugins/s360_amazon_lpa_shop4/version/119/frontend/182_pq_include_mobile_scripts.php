...
/**
 * Hook 182: Adds elements to HTML-DOM-Structure.
 */
...
try {
    ...
    */
    ...
    if ($mode !== 'l') {
        ...
        $showListingExpressButton = $renderHelper->isListingExpressButtonPossible($oPlugin);
        $listingExpressButtonArticles = array();
        if($showListingExpressButton) {
            $listingExpressButtonArticles = $renderHelper->getListingExpressButtonArticles();
        }
        ...
        if ($checkoutPossible && isset($_SESSION['Warenkorb']) && count($_SESSION['Warenkorb']->PositionenArr) > 0) {
            ...
            // darf Amazon Pay nicht angeboten werden
            $bExclude = false;
            ...
            if ($bExclude) {
                ...
                if (Shop::getPageType() === PAGE_BESTELLVORGANG) {
                    ...
                    if ($step === 'accountwahl') {
                        if(!isset($configBestellschritte) || $configBestellschritte == 124 || $configBestellschritte == 12 || $configBestellschritte == 1234) {
        ...
        if ($showDetailExpressButton) {
             ...
             */
            $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_INSERT_METHOD_MOBILE];
            $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_INSERT_SELECTORS_MOBILE];
            if (!empty($pqSelectors)) {
                Shop::Smarty()->assign('lpa_button_type_class', 'lpa-pay-button')
                    ...
                    ->assign('lpa_express_button', 1)
                    ->assign('lpa_express_button_type', 'detail'); // enables special JS in the template
        ...
        if ($showListingExpressButton && !empty($listingExpressButtonArticles)) {
            $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_INSERT_METHOD_MOBILE];
            $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_INSERT_SELECTORS_MOBILE];
            if (!empty($pqSelectors)) {
                Shop::Smarty()->assign('lpa_button_type_class', 'lpa-pay-button')
                    ->assign('lpa_login_redirect_uri', Shop::getURL(true) . '/lpacheckout' . $language_suffix)// MUST BE SSL!
                    ->assign('lpa_button_type', $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_TYPE])
                    ->assign('lpa_button_color', $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_COLOR])
                    ->assign('lpa_button_size', $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_SIZE])
                    ->assign('lpa_required_fields_message', $oPlugin->oPluginSprachvariableAssoc_arr[S360_LPA_LANGKEY_EXPRESS_REQUIRED_FIELDS])
                    ->assign('lpa_express_button', 1) // enables special JS in the template
                    ->assign('lpa_express_button_type', 'listing')
                    ->assign('lpa_express_push_method', 'pushToBasket');
                foreach($listingExpressButtonArticles as $kArtikel) {
                    // insert kArtikel in the pq selector
                    $fullSelector = str_ireplace('#kArtikel#', $kArtikel, $pqSelectors);
                    if($fullSelector === $pqSelectors) {
                        // no replacement was made, this is per definition wrong and the insertion process will be skipped
                        continue;
                    }
                    ...
                    if(!pq('#quantity' . $kArtikel)->length) {
                        // no quantity input field for this article, skip
                        continue;
                    }
                    $selectionPositions = pq($fullSelector);
                    foreach ($selectionPositions as $position) {
                        $lpa_button_idx++;
                        $renderHelper->addPayButton();
                        Shop::Smarty()->assign('lpa_button_idx', $lpa_button_idx)
                            ->assign('lpa_express_button_key', $kArtikel);
