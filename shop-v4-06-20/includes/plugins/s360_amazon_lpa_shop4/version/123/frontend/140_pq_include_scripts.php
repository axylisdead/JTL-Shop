...
try {
    ...
    if ($mode !== 'l') {
        ...
        if ($showDetailExpressButton) {
            ...
            if (!empty($pqSelectors)) {
                Shop::Smarty()->assign('lpa_button_type_class', 'lpa-pay-button')
                    ...
                    ->assign('lpa_express_button_type', 'detail');
        ...
        if ($showListingExpressButton && !empty($listingExpressButtonArticles)) {
            $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_INSERT_METHOD];
            $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_EXPRESS_LISTING_INSERT_SELECTORS];
            if (!empty($pqSelectors)) {
                Shop::Smarty()->assign('lpa_button_type_class', 'lpa-pay-button')
                    ->assign('lpa_login_redirect_uri', Shop::getURL(true) . '/lpacheckout' . $language_suffix)// MUST BE SSL!
