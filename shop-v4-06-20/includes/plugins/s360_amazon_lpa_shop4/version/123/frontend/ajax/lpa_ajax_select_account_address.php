...
// die Antwort ist im JSON Format
    ...
    if(!isset($_COOKIE[S360_LPA_ADDRESS_CONSENT_TOKEN_COOKIE]) || empty($_COOKIE[S360_LPA_ADDRESS_CONSENT_TOKEN_COOKIE])) {
        throw new Exception("Missing address consent token.");
    }
...
// check if Packstation was selected as address.
    ...
    // check allowed country codes, note that we now force all country codes, though, due to Geoblocking-VO
    $countryCode = strtolower($result['CountryCode']);
    $countryCodesIso = array_map(function($country) {
        return strtolower($country->cISO);
    }, gibBelieferbareLaender(0, false, true));
    if(!in_array($countryCode, $countryCodesIso)) {
        /*
         * Found country in selected country which is not allowed.
         */
        echo json_encode(array('status' => 'error', 'code' => 'countrycode'));
        return;
    }
...
// get first and last name
    ...
    // used to test against invalid characters
    $sCity = utf8_decode($result['City']);

    // ensure that no characters got lost during decoding. if any non-ISO-8859-1 characters were in the data, the address feelds would now contain a question mark (?)
    if(strpos($sFirstName.$sLastName.$sStreet.$sStreetNr.$sCompanyName.$sCity, '?') !== false) {
        // we lost information on decoding
        echo json_encode(array('status' => 'error', 'code' => 'nonlatin'));
        return;
    }
    ...
    Jtllog::writeLog('LPA: Fehler bei Auswahl der Versandadresse: ' . $ex->getMessage(), JTLLOG_LEVEL_DEBUG);
