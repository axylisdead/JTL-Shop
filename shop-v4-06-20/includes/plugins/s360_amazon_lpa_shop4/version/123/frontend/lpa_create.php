...
Shop::set('lpa_plugin_page', 'create');
...
$stmt = 'SELECT * FROM ' . S360_LPA_TABLE_ACCOUNTMAPPING . ' WHERE cAmazonId LIKE :amazonId AND cVerifizierungsCode LIKE :verificationCode LIMIT 1';
$result = Shop::DB()->executeQueryPrepared($stmt, ['amazonId' => $amazonId, 'verificationCode' => $verificationCode], 1);
if (empty($result)) {
...
if ($nReturnValue) {
    ...
    if ($createMode === "Y" || ($createMode === "O" && $createAccount)) {
        ...
        $stmt = "SELECT kKundenWerbenKunden FROM tkundenwerbenkunden WHERE cEmail = :kndmail AND nRegistriert = 0";
        $oNeukunde = Shop::DB()->executeQueryPrepared($stmt, ['kndmail' => $knd->cMail], 1);

        if (isset($oNeukunde) && !empty($oNeukunde) && $oNeukunde->kKundenWerbenKunden > 0 && isset($Einstellungen['kundenwerbenkunden']['kwk_kundengruppen']) && (int) $Einstellungen['kundenwerbenkunden']['kwk_kundengruppen'] > 0) {
    ...
    if ($createMode === "Y" || ($createMode === "O" && $createAccount)) {
        ...
        if (isset($oNeukunde) && !empty($oNeukunde) && $oNeukunde->kKundenWerbenKunden > 0) {
            Shop::DB()->query("UPDATE tkunde SET fGuthaben=fGuthaben+" . (float) $Einstellungen['kundenwerbenkunden']['kwk_neukundenguthaben'] . " WHERE kKunde = " . (int) $knd->kKunde, 3);
            $updObj = new stdClass();
            $updObj->nRegistriert = 1;
            Shop::DB()->update('tkundenwerbenkunden', 'cEmail', $knd->cMail, $updObj);
        }
    ...
    // check all coupons, remove them if necessary (i.e. they might not be valid anymore now that we know the emailaddress, even for guests)
    $oKupons = array();
