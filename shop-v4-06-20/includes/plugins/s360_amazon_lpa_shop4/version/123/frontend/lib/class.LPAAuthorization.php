...
class LPAAuthorization {
    ...
    private function fillFromArray($array) {
        ...
        $this->fAuthorizationAmount = (float) $array['AuthorizationAmount']['Amount'];
        $this->cAuthorizationCurrencyCode = $array['AuthorizationAmount']['CurrencyCode'];
        $this->fCapturedAmount = (float) $array['CapturedAmount']['Amount'];
        ...
        if (!empty($expiration)) {
            ...
            $this->nAuthorizationExpirationTimestamp = (int) date_timestamp_get(new DateTime($expiration));
        }
