...
try {
    ...
    if (!$admAccount->logged()) {
        echo json_encode(array('status' => 'fail', 'error' => "Fehler - Admin-Login abgelaufen?"));
        exit;
    }
    ...
    if(empty($config['secret_key'])) {
        $savedConfig = $controller->getConfig();
        if(!empty($savedConfig) && !empty($savedConfig['secret_key'])) {
            $config['secret_key'] = $savedConfig['secret_key'];
        }
    }
