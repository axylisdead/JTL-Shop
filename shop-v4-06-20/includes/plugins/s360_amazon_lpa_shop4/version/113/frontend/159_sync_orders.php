...
Jtllog::writeLog("LPA:  WaWi-Abgleich für Bestellung {$oBestellungNeu->cBestellNr}: Capture States: ".print_r($capStates, true)." State der Bestellung: ".print_r($oBestellungNeu->cStatus, true), JTLLOG_LEVEL_DEBUG);
...
if (in_array($oBestellungNeu->cStatus, $capStates)) {
    ...
    try {
        ...
        if(empty($orid)) {
            Jtllog::writeLog("LPA: WaWi-Abgleich für Bestellung {$oBestellungNeu->cBestellNr}: Keine passende Order-ID im Plugin gefunden. Bestellung wird ignoriert.", JTLLOG_LEVEL_NOTICE);
        } elseif ($capMode === 'shipment') {
            ...
            Jtllog::writeLog("LPA: WaWi-Abgleich: Captures für OrderReference {$orid} werden nicht durch WaWi-Abgleich ausgelöst, sondern manuell oder sofort bei der Autorisierung.", JTLLOG_LEVEL_DEBUG);
        }
    } catch (Exception $e) {
