...
{
    /**
     * @param RMAHistoryRepository|null $RMAHistoryRepository
     */
    public function __construct(public ?RMAHistoryRepository $RMAHistoryRepository = null)
    {
        $this->RMAHistoryRepository = $RMAHistoryRepository ?? new RMAHistoryRepository();
    }
    ...
    public function detectEvents(RMADomainObject $originalDO, RMADomainObject $modifiedDO): array
    {
        ...
        foreach ($modifiedData as $key => $modifiedValue) {
            ...
            if (($originalData[$key] !== $modifiedValue) && $eventName !== ''
    ...
    /**
     * @param RMAHistoryEventData $eventData
     * @return void
        ...
        foreach ($eventData->modifiedDO->getRMAItems()->uniqueArrayKeys() as $key => $modifiedItem) {
            if (isset($originalItems[$key])) {
                ...
                if ($modifiedItem->quantity !== $originalItem->quantity) {
                    ...
                    $this->RMAHistoryRepository->insert(
                        $domainObject
                    );
                }
    ...
    {
        $historyEvents = [];
        foreach ($this->RMAHistoryRepository->getList(
            ['rmaID' => $rma->id]
        ) as $historyEvent) {
     ...
     * @since 5.3.0
     */
    public function getLocalizedEventDataAsObject(
        ...
        if ($eventDataDomainObject->shippingNotePosID > 0 && $eventDataDomainObject->productID > 0) {
            $productName = $this->RMAHistoryRepository->getProductNameFromDB(
    ...
    private function convertAddressToString(array $addressArray): string
    {
        ...
        if (isset($addressArray['companyName']) && $addressArray['companyName'] !== '') {
        ...
        if (isset($addressArray['academicTitle']) && $addressArray['academicTitle'] !== '') {
        ...
        if (isset($addressArray['street']) && $addressArray['street'] !== '') {
            $result .= $addressArray['street'];
            if (isset($addressArray['houseNumber']) && $addressArray['houseNumber'] !== '') {
                $result .= ', ' . $addressArray['houseNumber'];
            }
            if (isset($addressArray['addressAdditional']) && $addressArray['addressAdditional'] !== '') {
                $result .= ' ' . $addressArray['addressAdditional'];
            }
            $result .= '<br>';
        }

        if (isset($addressArray['postalCode']) && $addressArray['postalCode'] !== '') {
        ...
        if (isset($addressArray['countryISO']) && $addressArray['countryISO'] !== '') {
        ...
        if (isset($addressArray['mobilePhone']) && $addressArray['mobilePhone'] !== '') {
            $result .= '<br>' . $addressArray['mobilePhone'] . ', ';
        }
        if (isset($addressArray['fax']) && $addressArray['fax'] !== '') {
            $result .= '<br>' . $addressArray['fax'] . ', ';
        }
        if (isset($addressArray['mail']) && $addressArray['mail'] !== '') {
