...
class PriceRange
{
    ...
    private function loadPriceRange(): void
    {
        ...
        if (Shop::getSettingValue(\CONF_GLOBAL, 'consistent_gross_prices') === 'Y') {
            $tempMwSt = $this->db->getSingleObject('SELECT fMwSt 
            FROM tartikel 
            WHERE kArtikel = :pid', ['pid' => (int)$this->productData->kArtikel]);
