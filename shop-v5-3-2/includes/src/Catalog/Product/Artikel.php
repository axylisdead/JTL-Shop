...
class Artikel implements RoutableInterface
{
    ...
    private function getStockDisplay(): self
    {
        ...
        if ($this->cLagerBeachten === 'Y') {
            ...
            if ($this->fLagerbestand >= (int)$this->getConfigValue('global', 'artikel_lagerampel_gruen')
                || ($this->cLagerKleinerNull === 'Y'
                    && $this->getConfigValue('global', 'artikel_ampel_lagernull_gruen') === 'Y')
            ) {
    ...
    public function getSimilarProducts(): array
    {
        ...
        $customerGroupID = $this->kKundengruppe ?? $this->getCustomerGroup()->getID();
        if ((int)$this->getConfigValue('artikeldetails', 'artikeldetails_aehnlicheartikel_anzahl') > 0) {
            $limitSQL = ' LIMIT '
                        . (int)$this->getConfigValue('artikeldetails', 'artikeldetails_aehnlicheartikel_anzahl');
        }
    ...
    public function gibMwStVersandString($net): string
    {
        ...
        if ($this->cVersandklasse !== null
            ...
            && $this->getConfigValue('global', 'global_versandklasse_anzeigen') === 'Y'
        ) {
