...
class IOMethods
{
     ...
     * @param RMAReasonService|null $rmaReasonService
    ...
    public function __construct(
        ...
        private ?RMAReasonService  $rmaReasonService = null,
        private ?RMAReturnAddressService $rmaReturnAddressService = null
    ) {
        $this->db                        = $this->db ?? Shop::Container()->getDB();
        $this->cache                     = $this->cache ?? Shop::Container()->getCache();
        $this->rmaService              ??= new RMAService();
        $this->rmaReasonService        ??= new RMAReasonService();
        $this->rmaReturnAddressService ??= new RMAReturnAddressService();
    }
    ...
    public function rmaItems(array $args = []): IOResponse
    {
        ...
        foreach ($param['quantity'] as $key => $quantity) {
            ...
            $rmaItems->append(new RMAItemDomainObject(
                ...
                reason: $this->rmaReasonService->getReason(
                    ...
                    languageID: Typifier::intify($languageID ?? 0)
                )
            ));
        }
