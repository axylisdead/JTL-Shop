...
function getDBStruct(bool $extended = false, bool $clearCache = false): array
{
    ...
    return (new Structure($db, Shop::Container()->getCache(), $info))->getDBStruct($extended, $clearCache);
}
...
function getDBFileStruct(): array
{
    ...
    return (new Structure($db, Shop::Container()->getCache(), $info))->getDBFileStruct();
}
...
function createDBStructError(string $msg, bool $engineError = false): stdClass
{
    ...
    return (new Structure($db, Shop::Container()->getCache(), $info))->createDBStructError($msg, $engineError);
}
...
function compareDBStruct(array $dbFileStruct, array $dbStruct): array
{
    ...
    return (new Structure($db, Shop::Container()->getCache(), $info))->compareDBStruct($dbFileStruct, $dbStruct);
}
...
function doMigrateToInnoDB_utf8(
    ...
    $info      = new Info($db);
    $check     = new Check($db);
    $structure = new Structure($db, Shop::Container()->getCache(), $info);
    $innodb    = new InnoDB($db, $info, $check, $structure, Shop::Container()->getGetText());
